...
function(reproducer::BitstringReproducer)(key::String, parent::BitstringGeno)
    # new_genes = Bool[]
    # for bit in parent.genes
    #     bit = rand(reproducer.rng) < reproducer.mutrate ? !bit : bit
    #     push!(new_genes, bit)
    # end
    new_genes = [rand(reproducer.rng) < reproducer.mutrate ? !bit : bit
                 for bit in parent.genes]
